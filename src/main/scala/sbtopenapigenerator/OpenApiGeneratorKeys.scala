...
trait OpenApiGeneratorKeys {
  ...
  final val verbose = settingKey[Boolean]("The verbosity of generation")
  final val validateSpec = settingKey[Boolean]("Whether or not an input specification should be validated upon generation.")
  final val generatorName = settingKey[String]("The name of the generator which will handle codegen. (see \"openApiGenerators\" task)")
  final val templateDir = settingKey[Option[String]]("The template directory holding a custom template.")
  ...
  final val skipOverwrite = settingKey[Boolean]("Specifies if the existing files should be overwritten during the generation.")
  ...
  final val library = settingKey[Option[String]]("Reference the library template (sub-template) of a generator.")
  final val gitHost = settingKey[Option[String]]("Git host, e.g. gitlab.com.")
  final val gitUserId = settingKey[Option[String]]("Git user ID, e.g. openapitools.")
  final val gitRepoId = settingKey[Option[String]]("Git repo ID, e.g. openapi-generator.")
  final val releaseNote = settingKey[Option[String]]("Release note, default to 'Minor update'.")
  final val httpUserAgent = settingKey[Option[String]]("HTTP user agent, e.g. codegen_csharp_api_client, default to 'OpenAPI-Generator/{packageVersion}}/{language}'")
  final val reservedWordsMappings = settingKey[Map[String, String]]("Specifies how a reserved name should be escaped to.")
  final val ignoreFileOverride = settingKey[Option[String]]("Specifies an override location for the .openapi-generator-ignore file. Most useful on initial generation.")
   ...
   */
  final val generateModelTests = settingKey[Boolean]("Defines whether or not model-related _test_ files should be generated.")
   ...
   * refer to it via [ignoreFileOverride].
   */
  final val generateModelDocumentation = settingKey[Boolean]("Defines whether or not model-related _documentation_ files should be generated.")
   ...
   * refer to it via [ignoreFileOverride].
   */
  final val generateApiTests = settingKey[Boolean]("Defines whether or not api-related _test_ files should be generated.")
   ...
   * refer to it via [ignoreFileOverride].
   */
  final val generateApiDocumentation = settingKey[Boolean]("Defines whether or not api-related _documentation_ files should be generated.")
  final val withXml = settingKey[Boolean]("A special-case setting which configures some generators with XML support. In some cases, this forces json OR xml, so the default here is false.")

  final val logToStderr = settingKey[Boolean]("To write all log messages (not just errors) to STDOUT")
   ...
   */
  final val enablePostProcessFile = settingKey[Boolean]("To enable the file post-processing hook. This enables executing an external post-processor (usually a linter program).")

  final val skipValidateSpec = settingKey[Boolean]("To skip spec validation. When true, we will skip the default behavior of validating a spec before generation.")
   ...
   */
  final val generateAliasAsModel = settingKey[Boolean]("To generate alias (array, list, map) as model")
